...
Experimenter-controlled assignment (Between-Subjects) via URL parameters (?condition=control vs ?condition=ai), rather than algorithmic randomization.

## Stimuli

Six pairs of high-fidelity UI mockups were created reflecting common interfaces within the Iqra University portal context:

1. Information Density (Course Schedule)
2. Data Visualization (HEC Attendance Warning)
3. Financial Overview (Fee Voucher)
4. Campus Event (Visual Dominance)
5. Interaction (QEC Faculty Evaluation)
6. Navigation Hierarchy (Digital Library)
...
1. Participants accessed the experiment via a URL that assigned them to one of two conditions using URL parameters.
...
- Data stored via edge-computed Firebase Firestore batch payloads, with isolated localStorage fallbacks in case of network failure.
